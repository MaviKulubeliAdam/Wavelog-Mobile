...
// ─────────────────────────────────────────────────────────────────────────────
// DXCC tab — extracted from statistics_screen.dart to keep that file under the
// project's line budget. Presentation-only extraction; provider/calculation
...
final dxccStatsProvider = FutureProvider<_DxccData>((ref) async {
...
class DxccTab extends ConsumerWidget {
  const DxccTab({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final l10n = context.l10n;
    final async = ref.watch(dxccStatsProvider);
    ...
    return async.when(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        _LegendDot(Icons.check_circle,   semantic.confirmed, l10n.dxccLegendConfirmed),
                        const SizedBox(width: 14),
                        _LegendDot(Icons.hourglass_empty, Colors.amber,      l10n.dxccLegendPending),
                        const SizedBox(width: 14),
                        _LegendDot(Icons.cancel,          semantic.needed,   l10n.dxccLegendNotWorked),
                      ],
                    ),
                  ],
                ),
      ...
      error: (e, _) => ErrorView(
        ...
        onRetry: () => ref.invalidate(dxccStatsProvider),
      ),
    );
  }
}
...
class _ContinentTile extends StatelessWidget {
            ...
            return Column(
              children: [
                ListTile(
                  ...
                  leading: _statusIcon(context, e.status),
                  title: Text(
                    e.country,
                    style: TextStyle(
  ...
  Widget _statusIcon(BuildContext context, _DxccStatus status) {
    final semantic = context.semanticColors;
    return switch (status) {
      _DxccStatus.confirmed => Icon(Icons.check_circle,
          color: semantic.confirmed, size: 18),
      _DxccStatus.pending => const Icon(Icons.hourglass_empty,
          color: Colors.amber, size: 18),
      _DxccStatus.notWorked => Icon(Icons.cancel,
          color: semantic.needed.withValues(alpha: 0.6), size: 18),
    };
  }
}
